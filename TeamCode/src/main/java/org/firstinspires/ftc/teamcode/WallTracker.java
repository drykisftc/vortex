...
public class WallTracker extends Tracker {
    ...
    private int wheel2wheelDist = 10;
    private int wheelDiameter = 10;
    private int ratio = 2500; //degrees of wheel turn to cm
    private int GetangleDistance = 200;
    ...
    public void moveByEncoder(int dist){
        int lD = leftWheel.getCurrentPosition();
        int rD = rightWheel.getCurrentPosition();
        int lDt = leftWheel.getCurrentPosition() - lD;
        int rDt = rightWheel.getCurrentPosition() - rD;
        while (lDt < dist && rDt < dist) {
            leftWheel.setPower(0.5);
            rightWheel.setPower(0.5);
            lDt = leftWheel.getCurrentPosition() - lD;
            rDt = rightWheel.getCurrentPosition() - rD;
        }
    }
    public void moveByEncoder(double dist){
        int lD = leftWheel.getCurrentPosition();
        int rD = rightWheel.getCurrentPosition();
        int lDt = leftWheel.getCurrentPosition() - lD;
        int rDt = rightWheel.getCurrentPosition() - rD;
    ...
    public void moveByEncoder(float dist) {
        int lD = leftWheel.getCurrentPosition();
        int rD = rightWheel.getCurrentPosition();
        int lDt = leftWheel.getCurrentPosition() - lD;
        int rDt = rightWheel.getCurrentPosition() - rD;
    ...
    public double getAngle() {
        double angle = 1;
        double raw1 = getHistoryDistanceAverage();
        int lD = leftWheel.getCurrentPosition();
        int rD = rightWheel.getCurrentPosition();
        int lDt = leftWheel.getCurrentPosition()-lD;
        int rDt = rightWheel.getCurrentPosition()-rD;
        while(lDt<GetangleDistance&&rDt<GetangleDistance){
            leftWheel.setPower(0.5);
            rightWheel.setPower(0.5);
            lDt = leftWheel.getCurrentPosition()-lD;
            rDt = rightWheel.getCurrentPosition()-rD;
        }
        double raw2 = getHistoryDistanceAverage();
        double delta = raw2-raw1;
        double distancetransversed = (lDt + rDt)/2;
        double distInCM = distancetransversed/ratio;
        double radian = Math.atan(delta/distInCM);
        angle = radian *180/Math.PI;
        return angle;
    }
    //Calculate distance using angle
    public double getDistanceFromAngle(double angle) {
        double raw = getHistoryDistanceAverage();
        double wd = Math.cos(angle*raw);
        double wdc = wd/ratio;
        return wdc;
    }
    //Correct robot path
    public int correct(int cs, double angle, double wd) {
        if (angle < 0) {
            double cAngle = targetangle + angle;
        }else{
            //double cAngle = targetangle - angle;
        }
        //turn cAngle degrees
        ...
        double distc = Math.sqrt(wd - targetDistance);
        //change distance to angle
        double dist = distc*ratio;
        //move using encoder
        moveByEncoder(dist);
        return cs + 1;
    }
